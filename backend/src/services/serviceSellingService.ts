...
export const createServiceSelling = async (
  sellerId: string,
  payload: any
) => {
...
export const getServiceSellingById = async (id: string) => {
  return ServiceSelling.findById(id).populate("categoryId", "name type");
};

export const updateServiceSelling = async (
  id: string,
  sellerId: string,
  payload: any
) => {
  const existing = await ServiceSelling.findOne({ _id: id, sellerId });
  ...
  return ServiceSelling.findByIdAndUpdate(
    id,
...
export const deleteServiceSelling = async (
  id: string,
  sellerId: string
) => {
  const deleted = await ServiceSelling.findOneAndDelete({
    _id: id,
    sellerId,
  });
  if (!deleted) throw new AppError("Forbidden", 403);
  return deleted;
};

export const canModifyServiceSelling = async (
  id: string,
  userId: string,
  role: Role
) => {
