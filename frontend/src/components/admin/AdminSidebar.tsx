...
const AdminSidebar: React.FC = () => {
  ...
  return (
    <aside
      className="group/sidebar fixed left-0 top-0 z-40 flex h-screen w-[68px] flex-col border-r border-slate-800/60 bg-slate-950 transition-all duration-300 ease-in-out hover:w-[240px]"
    >
      {/* Logo area */}
      <div className="flex h-16 items-center gap-3 border-b border-slate-800/60 px-4">
        ...
        <span className="whitespace-nowrap text-lg font-bold text-white opacity-0 transition-opacity duration-200 group-hover/sidebar:opacity-100">
          Bazaaro
        </span>
      </div>

      {/* Navigation */}
      <nav className="flex-1 space-y-1 overflow-y-auto overflow-x-hidden px-3 py-4 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-slate-800">
        {navItems.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
      ...
      <div className="border-t border-slate-800/60 px-3 py-3">
        <button
          ...
          className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-slate-400 transition-all duration-200 hover:bg-rose-500/10 hover:text-rose-400"
        >
