...
import {
  ...
  type ProductReportCategory,
  type ProductReportDatePreset,
...
interface ProductReportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGenerate: (options: ProductReportOptions) => void;
  isGenerating?: boolean;
}

const defaultCategories: ProductReportCategory[] = PRODUCT_REPORT_CATEGORIES.filter((item) => item.primary).map((item) => item.value);
...
const ProductReportModal: React.FC<ProductReportModalProps> = ({
  ...
  const [selectedCategories, setSelectedCategories] = useState<ProductReportCategory[]>(defaultCategories);
  const [datePreset, setDatePreset] = useState<ProductReportDatePreset>('30d');
  ...
  const toggleCategory = (value: ProductReportCategory) => {
  ...
  return (
    <AdminModal isOpen={isOpen} onClose={onClose} title="Generate Product PDF Report" size="lg">
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            {PRODUCT_REPORT_CATEGORIES.map((item) => {
        ...
        <div>
          ...
          <div className="mt-2 grid gap-3 sm:grid-cols-2">
            <select
              value={datePreset}
              onChange={(event) => setDatePreset(event.target.value as ProductReportDatePreset)}
