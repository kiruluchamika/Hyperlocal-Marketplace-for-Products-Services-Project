    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const geoValidServices = data.data.services.filter(
          (service) => service.location?.coordinates && Number.isFinite(service.location.coordinates[0]) && Number.isFinite(service.location.coordinates[1])
        );

        setResults(geoValidServices);
      } catch {
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-6 px-4 sm:px-6 lg:grid-cols-3 lg:px-8">
        <div className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-card lg:col-span-1 lg:sticky lg:top-24 lg:h-fit">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Explore service providers around your selected location.</p>
          </div>
        ...
        <div className="space-y-4 lg:col-span-2">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-card">
            <div className="mb-3 flex items-center justify-between">
              ...
              <p className="text-xs text-slate-500">Showing only geo-mapped services</p>
            </div>
