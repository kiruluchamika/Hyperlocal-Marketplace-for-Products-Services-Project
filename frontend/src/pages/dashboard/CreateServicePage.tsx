...
import { FiCrosshair, FiPlus, FiSave, FiTrash2 } from 'react-icons/fi';
...
type PricingTypeOption = PricingType | '';
...
const getFieldClassName = (hasError: boolean) =>
  `input-field py-2 ${hasError ? 'border-red-300 focus:border-red-500 focus:ring-2 focus:ring-red-500/20' : ''}`;

const validateForm = (form: ServiceFormState, category: ICategory | undefined): FieldErrors => {
  ...
  if (!form.pricingType) {
    errors.pricingType = 'Please choose a pricing type.';
  }
  ...
  if ((form.latitude.trim() || form.longitude.trim()) && (!Number.isFinite(lat) || !Number.isFinite(lng))) {
    errors.coordinates = 'Latitude and longitude must both be valid numbers.';
  }

  if (form.images.length > MAX_IMAGES) {
    errors.images = `Maximum ${MAX_IMAGES} images are allowed.`;
  }
  ...
  if (category) {
    for (const attribute of category.attributes) {
      ...
      if (attribute.required && isMissing) {
        errors[`attribute:${attribute.fieldName}`] = `${attribute.fieldName} is required.`;
      }
    }
  }
...
const renderAttributeInput = (
  ...
  if (attribute.fieldType === 'select') {
    return (
      <select className={getFieldClassName(!!error)} value={typeof value === 'string' ? value : ''} onChange={(event) => onChange(event.target.value)} onBlur={onBlur}>
...
const CreateServicePage: React.FC = () => {
  ...
  const fieldErrors = React.useMemo(() => validateForm(form, selectedCategory), [form, selectedCategory]);
  ...
  const shouldShowError = React.useCallback(
    (field: string) => !!fieldErrors[field] && (submitAttempted || !!touchedFields[field]),
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const payload = {
      ...
      locationText: form.locationText.trim() || form.district.trim(),
      images: form.images,
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card">
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
            <div className="md:col-span-2">
              ...
              <input className={getFieldClassName(shouldShowError('title'))} value={form.title} onChange={(event) => setForm((prev) => ({ ...prev, title: event.target.value }))} onBlur={() => markTouched('title')} maxLength={100} />
              {shouldShowError('title') && <p className="mt-1.5 text-sm text-red-500">{fieldErrors.title}</p>}
            </div>
            <div className="md:col-span-2">
              <label className="mb-1 block text-xs font-medium text-slate-500">Description</label>
              <textarea className={getFieldClassName(false)} rows={5} value={form.description} onChange={(event) => setForm((prev) => ({ ...prev, description: event.target.value }))} maxLength={2000} />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-slate-500">Category</label>
              <select className={getFieldClassName(shouldShowError('categoryId'))} value={form.categoryId} onChange={(event) => setForm((prev) => ({ ...prev, categoryId: event.target.value }))} onBlur={() => markTouched('categoryId')}>
              ...
              {shouldShowError('categoryId') && <p className="mt-1.5 text-sm text-red-500">{fieldErrors.categoryId}</p>}
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-slate-500">Pricing Type</label>
              <select className={getFieldClassName(shouldShowError('pricingType'))} value={form.pricingType} onChange={(event) => setForm((prev) => ({ ...prev, pricingType: event.target.value as PricingTypeOption }))} onBlur={() => markTouched('pricingType')}>
                <option value="">Choose pricing type</option>
                <option value="FIXED">Fixed Price</option>
                <option value="HOURLY">Hourly Rate</option>
              </select>
              {shouldShowError('pricingType') && <p className="mt-1.5 text-sm text-red-500">{fieldErrors.pricingType}</p>}
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-slate-500">Price {form.pricingType === 'HOURLY' ? '(per hour)' : ''}</label>
              <input type="number" min={0} className={getFieldClassName(shouldShowError('price'))} value={form.price} onChange={(event) => setForm((prev) => ({ ...prev, price: event.target.value }))} onBlur={() => markTouched('price')} />
              {shouldShowError('price') && <p className="mt-1.5 text-sm text-red-500">{fieldErrors.price}</p>}
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-slate-500">Location Text</label>
              <input className={getFieldClassName(false)} value={form.locationText} onChange={(event) => setForm((prev) => ({ ...prev, locationText: event.target.value }))} placeholder="Ja-Ela, Colombo, online, at customer location..." maxLength={120} />
            </div>
          </div>
        </section>
              ...
              {selectedCategory.attributes.map((attribute) => (
                <div key={attribute.fieldName}>
                  ...
                  {renderAttributeInput(
                    attribute,
                    form.attributeValues[attribute.fieldName],
                    (value) => handleAttributeChange(attribute.fieldName, value),
                    () => markTouched(`attribute:${attribute.fieldName}`),
                    shouldShowError(`attribute:${attribute.fieldName}`) ? fieldErrors[`attribute:${attribute.fieldName}`] : undefined
                  )}
                  {shouldShowError(`attribute:${attribute.fieldName}`) && (
                    <p className="mt-1.5 text-sm text-red-500">{fieldErrors[`attribute:${attribute.fieldName}`]}</p>
                  )}
                </div>
              ))}
            </div>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card">
          ...
          <div className="mt-4 flex flex-col gap-3 sm:flex-row">
            <div className="flex-1">
              ...
              <div className={`flex h-[42px] w-full items-center rounded-xl border border-dashed px-4 text-sm ${shouldShowError('images') ? 'border-red-300 bg-red-50 text-red-600' : 'border-slate-300 bg-slate-50 text-slate-500'}`}>
                {form.images.length} / {MAX_IMAGES} images selected
              </div>
              {shouldShowError('images') && <p className="mt-1.5 text-sm text-red-500">{fieldErrors.images}</p>}
            </div>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card">
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
            <div>
              ...
              <select className={getFieldClassName(shouldShowError('district'))} value={form.district} onChange={(event) => handleDistrictChange(event.target.value)} onBlur={() => markTouched('district')}>
              ...
              {shouldShowError('district') && <p className="mt-1.5 text-sm text-red-500">{fieldErrors.district}</p>}
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-slate-500">Address (optional)</label>
              <input className={getFieldClassName(false)} value={form.address} onChange={(event) => setForm((prev) => ({ ...prev, address: event.target.value }))} />
            </div>
            <div className="md:col-span-2">
              <div className={`rounded-xl border p-3 ${shouldShowError('coordinates') ? 'border-red-200 bg-red-50' : 'border-slate-200 bg-slate-50'}`}>
                ...
                {shouldShowError('coordinates') && <p className="mt-2 text-sm text-red-500">{fieldErrors.coordinates}</p>}
              </div>
            </div>
          </div>
        </section>
