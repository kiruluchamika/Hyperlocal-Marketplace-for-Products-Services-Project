...
const CheckoutForm: React.FC<CheckoutFormProps> = ({ amount, currency, onClose, onSuccess }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col">
      ...
      <div className="-mx-6 -mb-6 flex shrink-0 items-center justify-end gap-3 border-t border-slate-200 bg-white px-6 py-4">
        <button
          type="button"
          onClick={onClose}
...
const OrderStripeCheckoutModal: React.FC<CheckoutModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-start justify-center overflow-y-auto bg-slate-900/60 px-4 py-6 sm:py-8">
      <div className="flex max-h-[calc(100vh-3rem)] w-full max-w-xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl sm:max-h-[calc(100vh-4rem)]">
